...
for i, txt in enumerate(realworld_Hgraphs):
    ax[0].scatter(mean_k1s[i], mean_k2s[i], 
...
#ax[0].set_xlim([0, 50])
#ax[0].set_ylim([0, 50])
ax[0].plot([0, 1], [0, 1], transform=ax[0].transAxes, color="gray", linestyle='--')
ax[0].loglog()
...
for i, txt in enumerate(realworld_Hgraphs):
    ax[1].scatter(mean_d1s[i], mean_d2s[i], 
               marker= data_marker_dict[txt], 
               color = data_type_color[data_type_dict[txt]],
               label = txt)
...
ax[1].set_title(r"Mean Degree")
#ax[1].set_xlim([0, 2500])
#ax[1].set_ylim([0, 2500])
ax[1].loglog()
ax[1].plot([0, 1], [0, 1], transform=ax[1].transAxes, color="gray", linestyle='--')
#label_outliers(ax[1], mean_d1s, mean_d2s, labels, threshold_degree)

ax[1].legend(ncol = 2,fontsize=10,  bbox_to_anchor=(1,1))

plt.savefig("figures/MEANS_scatter_log.pdf",bbox_inches='tight')
