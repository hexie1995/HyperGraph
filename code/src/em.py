...
class EM:
    ...
    def predict_v1(self, new_candidate_edges):
        ...
        # sequences of edge sizes, in order
        edge_sizes = H_comp.edges.size.asnumpy()
        
        # sequence of number of edges in hypergraph (one is added in each timestep)
        # NOTE: might need to modify this if we decide we want to formally allow hypergraphs to init with more than a single edge
        edge_counts = np.arange(1, len(xgi.to_hyperedge_list(H_comp)))
        
        # sequence of number nodes in hypergraph at each timestep
        # grows in time as nodes get added
        node_counts = []
        nodes = set()
        for x in xgi.to_hyperedge_list(H_comp):
            nodes = nodes.union(x)
            node_counts.append(len(nodes))
        ...
        IX = np.tril(IX) 
        np.fill_diagonal(IX, 0) 
        intersection_sizes = IX
        
        # number of novel nodes per new edge
        # NOTE: could this be obtained from node_counts without recalculating, right?
        novel_node_vec = []
        seen = set()
        for e in H_comp.edges.members():
        ...
        # number of nodes from hypergraph added per new edge, depending on which old edge was sampled
        # entry [i, j] is the number of nodes that must have been added to form i if j was sampled 
        other_nodes_mat = (edge_sizes - num_novel_nodes)[:,np.newaxis] - intersection_sizes
        other_nodes_mat = np.tril(other_nodes_mat) 
        np.fill_diagonal(other_nodes_mat, 0) 
    
        
        s1 = self.edge_sample_likelihood(selected     = intersection_sizes,
                                         total        = edge_sizes[np.newaxis,:], 
                                         eta          = self.pars["eta"])
        
        s2 = self.novel_nodes_likelihood(num_novel_nodes, beta = self.pars["beta"])
        
        s3 = self.nodes_from_hypergraph_likelihood(other_nodes_mat, gamma = self.pars["gamma"])
        
        s3 = s3*node_counts[:,np.newaxis]**(-1.0*other_nodes_mat)
        
        PRED = s1*s2*s3
        # *s2*s3
        
        ix = PRED.sum(axis = 1) == 0  
        PRED[ix,:] = np.eye(PRED.shape[0])[ix,:]
        
        PRED_V1 = PRED.sum(axis = 1)[:, np.newaxis]
        
        PRED_V2 = np.mean(PRED[-new_edge_counts:, :base_edges], axis=1)
