...
# +
print("dataset", "&", "eta", "&",  "beta", "&", "gamma", "&", "time", "&", "converge", r"\\" )
ETA = []
BETA = []
GAMMA = []
...
with open('figures/parameters.csv','w', newline='') as file:
    ...
    for data_name in realworld_Hgraphs:
        ...
        try:
            with open('sem_results/res_CS2_{}.pkl'.format(data_name), "rb") as f:
                d = pickle.load(f)
            ...
            eta_value = np.mean(d["eta"][-50:])
            beta_value = expectation(np.mean(d["beta"][-50:], axis=0))
            gamma_value = expectation(np.mean(d["gamma"][-50:], axis=0))
            
            if eta_value < 0.001:
                str_eta = '%.2E' % Decimal(np.mean(d["eta"][-50:]))
            elif eta_value >=0.001:
                str_eta = round(eta_value, 3)
            
            if beta_value < 0.001:
                str_beta =  '%.2E' % Decimal(expectation(np.mean(d["beta"][-50:], axis=0)))
            elif beta_value >=0.001:
                str_beta = round(beta_value, 3)
            
            if gamma_value < 0.001:
                str_gamma = '%.2E' % Decimal(expectation(np.mean(d["gamma"][-50:], axis=0)))
            elif gamma_value >=0.001:
                str_gamma = round(gamma_value, 3)

            print(d["dataset"],"&",  str_eta ,"&",  str_beta, "&", str_gamma,"&", \
                  round(d["time"],1), "&", d["converge"],  r"\\")          
            
#             print(d["dataset"],"&",  '%.2E' % Decimal(np.mean(d["eta"][-50:])),"&",  \
#                   '%.2E' % Decimal(expectation(np.mean(d["beta"][-50:], axis=0))), "&", \
#                   '%.2E' % Decimal(expectation(np.mean(d["gamma"][-50:], axis=0))),"&",  \
#                   '%.2E' % Decimal(d["time"]), "&", d["converge"],  r"\\")
...
ax[0].set_xlabel(r"log($\mu_{\beta}$)")
ax[0].set_ylabel(r"$\eta$")
ax[1].set_xlabel(r"log($\mu_{\gamma}$)")
#ax[1].set_ylabel("ETA")
